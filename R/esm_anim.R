                     ...
                     vis_options = NULL)
{
  ...
  saveHTML({
    for (i in min(data[["week_esmvis"]]):max(data[["week_esmvis"]]) ) {
      data_zoom <- filter(data, week_esmvis ==  i)
      ...
      print(overall_ts + zoom_ts - nw + plot_layout(ncol = 1, heights = c(1, 2)))
      print("bla")
    }
  }, interval = 2, htmlfile = paste("ESMvis_", Sys.time(), ".html", sep = ""),
  ani.width = 1150, ani.height = 640, verbose = FALSE, navigator = TRUE)
